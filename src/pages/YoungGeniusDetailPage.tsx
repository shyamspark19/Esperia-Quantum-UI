...
interface YoungGeniusDetailPageProps {
  onBack: () => void;
...
export default function YoungGeniusDetailPage({
  onBack,
  onNavigateToHome,
  onNavigateToContact,
      ...
      <header className="sticky top-0 z-50 bg-white/95 backdrop-blur-md border-b border-black/[0.06] transition-all duration-300">
        ...
        {mobileMenuOpen && (
          <div className="md:hidden bg-white border-b border-black/[0.08] px-6 py-6 flex flex-col gap-4 relative z-50 shadow-xl animate-in slide-in-from-top duration-300">
            <button type="button" className="text-left text-[#4A4A4A] hover:text-[#C5445A] text-sm py-1"
              onClick={() => { setMobileMenuOpen(false); if (onNavigateToWhatWeDo) onNavigateToWhatWeDo(); else onNavigateToHome('what-we-do'); }}>
              What We Do
            </button>
            <button type="button" className="text-left text-[#4A4A4A] hover:text-[#C5445A] text-sm py-1"
              onClick={() => { setMobileMenuOpen(false); onNavigateToHome('why-esperia'); }}>
              Why Esperia
            </button>
            <button type="button" className="text-left text-[#C5445A] font-semibold text-sm py-1"
              onClick={() => { setMobileMenuOpen(false); onBack(); }}>
              Our Products
            </button>
            <button type="button" className="text-left text-[#4A4A4A] hover:text-[#C5445A] text-sm py-1"
              onClick={() => { setMobileMenuOpen(false); onNavigateToHome('blogs'); }}>
              Blogs &amp; Newsletters
            </button>
            <button type="button" className="text-left text-[#C5445A] font-semibold pt-3 border-t border-slate-100 cursor-pointer text-sm"
        ...
        <section className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-center mb-12 sm:mb-16">
          <div className="lg:col-span-6 flex flex-col justify-center">
            ...
            <p className="text-[#555555] text-sm sm:text-base lg:text-[17px] font-normal leading-relaxed max-w-lg font-['Manrope',sans-serif]">
              AI-driven adaptive learning platform tailoring personalized education experiences at scale for learners, educators, and institutions nationwide.
            </p>
          </div>
          <div className="lg:col-span-6 flex justify-center lg:justify-end">
            <div className="w-full max-w-[620px] transition-transform duration-500 hover:scale-[1.02]">
              <img
                ...
                alt="Young Genius Program Platform"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div className="bg-[#F4F4F1] rounded-2xl p-4 sm:p-5 flex flex-col justify-between border border-slate-100 hover:shadow-xl transition-all duration-300 hover:-translate-y-1 group">
              ...
              <div className="flex items-center justify-between pt-1">
                ...
                <button type="button" onClick={() => onViewWatermelon ? onViewWatermelon() : onBack()}
            ...
            {/* Young Genius (current – active ring) */}
            ...
            <div className="bg-[#F4F4F1] rounded-2xl p-4 sm:p-5 flex flex-col justify-between border border-slate-100 hover:shadow-xl transition-all duration-300 hover:-translate-y-1 group">
              ...
              <div className="flex items-center justify-between pt-1">
                ...
                <button type="button" onClick={() => onViewIrisHr ? onViewIrisHr() : onBack()}
                  className="inline-flex items-center gap-1 text-[13px] font-semibold text-[#C5445A] hover:text-[#9e2e41] transition-colors cursor-pointer">
                  <span>View Details</span><span className="text-xs">&gt;</span>
                </button>
              </div>
            ...
            <div className="bg-[#F4F4F1] rounded-2xl p-4 sm:p-5 flex flex-col justify-between border border-slate-100 hover:shadow-xl transition-all duration-300 hover:-translate-y-1 group">
              ...
              <div className="flex items-center justify-between pt-1">
                ...
                <button type="button" onClick={() => onViewTasConnect ? onViewTasConnect() : onBack()}
                  className="inline-flex items-center gap-1 text-[13px] font-semibold text-[#C5445A] hover:text-[#9e2e41] transition-colors cursor-pointer">
                  <span>View Details</span><span className="text-xs">&gt;</span>
                </button>
              </div>
